...
def verify_token_type(actual_token:dict[str,str], expected_tokens:list[dict[str,str]])->None:
    """
    Purpose:
...
def process_class(
    ...
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'keyword','value':'class'}])
    ...
    # verify if tag is class else raise Exception
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'identifier'}])
    ...
    # verify if tag is class else raise Exception
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'symbol','value':'{'}])
...
def process_subroutine_declaration(
    ...
    current_index, token = process_parameter_list(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
    ...
    current_index, token = process_subroutine_body(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
...
def process_subroutine_body(
    ...
    while token['tag'] == 'keyword' and token['value'] == 'var':
        current_index, token = process_variable_declaration(output_file=output_file,token=token,token_list=token_list,current_index=current_index)

    current_index, token = process_statements(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
...
def process_statements(
    ...
    while token['tag'] == 'keyword' and (token['value'] == 'let' or token['value'] == 'if' or token['value'] == 'while' or token['value'] == 'do' or token['value'] == 'return'):
        if token['tag'] == 'keyword' and token['value'] == 'let':
            current_index, token = process_let_statement(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
        elif token['tag'] == 'keyword' and token['value'] == 'if':
            current_index, token = process_if_statement(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
        elif token['tag'] == 'keyword' and token['value'] == 'while':
            current_index, token = process_while_statement(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
        elif token['tag'] == 'keyword' and token['value'] == 'do':
            current_index, token = process_do_statement(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
        elif token['tag'] == 'keyword' and token['value'] == 'return':
            current_index, token = process_return_statement(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
...
def process_while_statement(
    ...
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'keyword','value':'while'}])
    ...
    current_index, token = process_expression(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
...
def process_if_statement(
    ...
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'keyword','value':'if'}])
...
def process_do_statement(
    ...
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'keyword','value':'do'}])
...
def process_return_statement(
    ...
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'keyword','value':'return'}])
...
def process_let_statement(
    ...
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'keyword','value':'let'}])
    ...
    # get next token
    token,current_index = get_next_token(token_list=token_list,i=current_index)
...
def process_expression(
    ...
    current_index, token = process_term(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
...
def process_term(
    ...
    if token['tag'] == 'integerConstant':
        ...
        if next_token['tag'] == 'symbol' and next_token['value'] == '[':
            ...
            current_index, token = get_next_token(token_list=token_list,i=current_index)
        elif next_token['tag'] == 'symbol' and next_token['value'] == '(':
            current_index, token = process_subroutine_call(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
        else:
            # write <identifier> var name </identifier>
            write_token(output_file=output_file,token=token)
...
def process_subroutine_call(
    ...
    if token['tag'] == 'symbol' and token['value'] == '(':
        ...
        current_index, token = process_expression_list(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
...
def process_expression_list(
    ...
    while token['tag'] == 'symbol' and token['value'] == ',':
        ...
        process_expression(output_file=output_file,token=token,token_list=token_list,current_index=current_index)
...
def process_variable_declaration(
    ...
    verify_token_type(actual_token=token,expected_tokens=[{'tag':'keyword','value':'var'}])
    ...
    if os.path.isdir(filename):
        ...
        for file in os.listdir(dir_name):
            ...
            if file.endswith("T.xml"):
                ...
                current_index = 0
                current_index, token = get_next_token(token_list=token_list,i=current_index)
                current_index, token = process_class(output_file=output_file_name,token=token,token_list=token_list,current_index=current_index)
    else:
        # get single filename
        ...
        # output file name
        output_file_name = f"{filename}.xml"
        token_list = generate_token_list(input_file=input_file)
        current_index = 0
        current_index, token = get_next_token(token_list=token_list,i=current_index)
